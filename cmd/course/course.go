...
func run(ctx context.Context) error {
	...
	logger = logger.Named("go_course_app")
	defer func() {_ = logger.Sync() }()

	pool, err := pgxpool.New(ctx, fmt.Sprintf("user=%s password=%s host=%s port=%s dbname=%s", os.Getenv("COURSE_DATABASE_USER"), 
	os.Getenv("COURSE_DATABASE_PASSWORD"), os.Getenv("COURSE_DATABASE_HOST"), os.Getenv("COURSE_DATABASE_PORT"), os.Getenv("COURSE_DATABASE_NAME") ))
	if err != nil {
		return err
	}
	defer pool.Close()
